...
(*
Теорема Кантора: для любого множества A, |P(A)| > |A|. Не существует сюръективной функции, отображающей A в Pow(A).

Функция из множества X в множество Y --- это правило, которое сопоставляет каждому элементу множества X ровно 1 элемент множества Y.
X называется доменом функции
Пусть дано множество A, построим его Powerset

Посколь f - сюръективная функция, то $\forall p \in Pow(A) \exists a \in A, f(a) = p$. Поскольку B состоит только из элементов A, оно является подмножеством A: $B \subseteq A$.
Следовательно, $B \in Pow(A)$.

Из сюръективности f следует, что для B как элемента Powerset существует элемент a, такой что $f(a) = B$. a может либо принадлежать B, либо не принадлежать B.
exists f:
*)
Export Relation.
