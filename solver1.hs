...
process_inner :: (Show a, Eq a) => Formula a -> [(a, Bool)] -> Maybe(Tree a) -> (Tree a, [(a, Bool)])
process_inner f atoms subtree =
  case f of
    F_atom a -> let atoms' = (a, True):atoms
                in
                  case subtree of
                    Nothing -> case (has_contradiction atoms') of
                                 Nothing -> (Open_leaf f, atoms')
                                 Just b -> (Closed_leaf f (F_atom b), atoms')
                    Just subtree' -> (One_child f subtree', atoms')
                        ...
                        in
                          case subtree of
                            Nothing -> case (has_contradiction atoms') of
                                         ...
                                         Just b -> (Closed_leaf f (F_atom b), atoms')
                            Just subtree' -> (One_child f subtree', atoms')
    F_neg f' -> let (subtree', atoms') = process_inner f' atoms subtree
                in
...
process f =
  let (tree, _) = process_inner f [] Nothing
  in
    tree
...
f11 :: Formula NoQuotes
f11 = F_neg (F_atom atom_A)

f2 :: Formula NoQuotes
f2 = F_neg (F_neg (F_atom atom_A))

f3 :: Formula NoQuotes
f3 = F_conj (F_atom atom_A) (F_neg (F_atom atom_A))
